...
function custom_woocommerce_pagination_query_string( $query_string ) {
    if ( is_product_category() ) {
        // Удаляем стандартный параметр пагинации
        unset( $query_string['paged'] );
        
        // Добавляем новый параметр для пагинации
        if ( isset( $_GET['page'] ) ) {
            $query_string['paged'] = intval( $_GET['page'] );
        }
    }
    return $query_string;
}
add_filter( 'request', 'custom_woocommerce_pagination_query_string' );

function custom_woocommerce_pagination_base() {
    global $wp_rewrite;
    $wp_rewrite->pagination_base = ''; // Устанавливаем пустую базу для пагинации
}
add_action( 'init', 'custom_woocommerce_pagination_base' );
